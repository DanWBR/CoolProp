...
* **New .NET 10 binding (``CoolProp.Net``)** — a NuGet package that carries native binaries for six desktop runtimes (``win-x64``, ``win-arm64``, ``linux-x64``, ``linux-arm64``, ``osx-x64``, ``osx-arm64``) under ``runtimes/<rid>/native/``, so a consuming application resolves the correct architecture automatically with no manual DLL placement.  It binds the flat C API in ``CoolProp/CoolPropLib.h`` through the ``LibraryImport`` source generator rather than SWIG, which makes it trimming- and NativeAOT-clean: a ``PublishAot`` executable calling ``PropsSI`` links and runs with no ``IL2xxx``/``IL3xxx`` warnings.  Every C ``long`` is marshalled as ``CLong`` so handles and error codes stay correct on both LLP64 (Windows) and LP64 (Linux/macOS).  This is additive — the existing SWIG C# wrapper (``CoolPropCsharp``) is unchanged.  32-bit Windows is not supported, being the only configuration where the C API uses ``__stdcall``.  See ``wrappers/DotNet/README.md``.
* **Shared-library install paths are now architecture-unique.**  The install folder previously carried an ``__arm64`` suffix only under MSVC, so on Linux and macOS an x86-64 build and an arm64 build both landed in ``shared_library/<System>/64bit`` and silently overwrote one another when the per-OS CI artifacts were merged.  Shared-library builds now additionally install to ``runtimes/<rid>/native/``, and the ``library_shared`` workflow builds the two architectures that were previously never produced at all (``linux-arm64`` and ``osx-x64``) and fails if any of the six is missing.  The legacy ``shared_library/<System>/<Bitness>bit`` layout is unchanged.
